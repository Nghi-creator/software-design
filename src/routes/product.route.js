...
import { ProductCommentService } from '../services/productComment.service.js';
...
router.get('/detail', async (req, res) => {
  ...
  if (req.session.authUser && product.seller_id === req.session.authUser.id) {
    rejectedBidders = await RejectedBidderService.getRejectedBidders(productId);
  }
...
router.post('/bid', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    BiddingService.sendBidNotifications(result, productUrl).catch(err => console.error('Email error:', err));

    res.json({
      success: true,
      newPrice: result.newCurrentPrice,
      highestBidderId: result.newHighestBidderId,
      isHighestBidder: result.newHighestBidderId === userId,
      isSold: result.productSold,
...
router.post('/comment', isAuthenticated, async (req, res) => {
  ...
  await ProductCommentService.createComment(productId, userId, content, parentId);
...
router.post('/order/upload-images', isAuthenticated, uploadImageMiddleware.array('images', 5), async (req, res) => {
  try {
...
router.post('/order/:orderId/complete-transaction', isAuthenticated, async (req, res) => {
  try {
    const orderId = req.params.orderId;
    const userId = req.session.authUser.id;
    await OrderService.skipRatingAndComplete(orderId, userId);
...
router.post('/reject-bidder', isAuthenticated, async (req, res) => {
  ...
  try {
    await RejectedBidderService.rejectBidder(productId, bidderId, sellerId);
...
router.post('/unreject-bidder', isAuthenticated, async (req, res) => {
  ...
  try {
    const result = await RejectedBidderService.unrejectBidder(productId, bidderId, sellerId);
